  ...
  if (videos && videos.length > 0) {
    videos.forEach(function(v) {
      ...
      if (v.thumbnail_url) {
        ...
        var ph = mk('div');
        ph.style.cssText = 'width:100%;height:100%;background:#111;display:flex;align-items:center;justify-content:center;font-size:1.5rem';
        ph.textContent = '🎬'; cell.appendChild(ph);
      }
    ...
    grid.innerHTML = "<div style='grid-column:1/-1;padding:30px;text-align:center;color:var(--text2)'>" + (emptyMsg || 'Няма видеа') + "</div>";
  }
  return grid;
}
...
async function renderProfile() {
  ...
  if (!STATE.user) {
    ...
    var lb2 = mk('button', 'btn-gold');
    lb2.style.cssText = 'width:auto;padding:12px 32px;margin:0 auto;display:block';
    lb2.textContent = 'Вход / Регистрация';
    lb2.onclick = function() { openModal('m-auth'); };
  ...
  if (!el('upl-avatar-inp')) {
    var ai2 = document.createElement('input'); ai2.type = 'file'; ai2.accept = 'image/*';
    ai2.id = 'upl-avatar-inp'; ai2.style.display = 'none';
    ai2.onchange = async function() {
      if (!this.files || !this.files[0]) return; showToast('Качване...');
      var file = this.files[0]; var ext = file.name.split('.').pop();
      var path = 'avatars/' + STATE.user.id + '.' + ext;
      try {
        ...
        STATE.user.avatar = url + '?t=' + Date.now();
  ...
  // Stats — показваме — веднага, зареждаме числата async
  ...
  [{ n: snVid, l: 'Видеа' }, { n: snFan, l: 'Фена' }, { n: snFol, l: 'Следва' }].forEach(function(s) {
    var st = mk('div'); var sl = mk('div', 'pst-l'); sl.textContent = s.l;
    st.appendChild(s.n); st.appendChild(sl); stats.appendChild(st);
  });

  var btns = mk('div', 'p-action-btns');
  var eb = mk('button', 'p-edit-btn'); eb.textContent = '✏️ Редактирай';
  eb.onclick = function() {
    var en = el('edit-name'); var ebi = el('edit-bio');
    if (en) en.value = STATE.user.name || '';
    if (ebi) ebi.value = STATE.user.bio || '';
    openModal('m-edit-profile');
  };
  var cb = mk('button', 'p-creator-btn'); cb.textContent = '🎬 Creator Панел';
  cb.onclick = function() { showPage('creator'); };
  btns.appendChild(eb); btns.appendChild(cb);

  hero.appendChild(avW); hero.appendChild(nm); hero.appendChild(hd);
  hero.appendChild(stats); hero.appendChild(bio); hero.appendChild(btns);
  wrap.appendChild(hero);

  // Tabs
  var tabs = mk('div', 'p-tabs'); var tabBtns = []; var grids = [];
  ['📹 Видеа', '❤️ Харесани', '🔖 Запазени', '💎 Премиум'].forEach(function(t, idx) {
    var tb = mk('button', 'p-tab' + (idx === 0 ? ' active' : ''));
    tb.textContent = t; tabs.appendChild(tb); tabBtns.push(tb);
  });
  wrap.appendChild(tabs);

  // Placeholder докато зареждаме
  var loadingDiv = mk('div');
  loadingDiv.style.cssText = 'padding:30px;text-align:center;color:var(--text2)';
  loadingDiv.textContent = 'Зарежда...';
  ...
  // Зареди видеа
  var myVids = [], likedVids = [], savedVids = [];
  ...
  // Провери дали все още сме на профил страница
  var active = document.querySelector('.page.active');
  if (!active || active.id !== 'page-profile') return;

  // Премахни loading
  if (loadingDiv.parentNode === wrap) wrap.removeChild(loadingDiv);

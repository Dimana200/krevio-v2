async function sendNotif(toUserId, type, text, videoId) {
  ...
  try {
    await STATE.sb.from('notifications').insert({
      user_id: toUserId, from_user_id: STATE.user.id,
      type: type, text: text, video_id: videoId || null, read: false
    });
  } catch(e) {}
}
...
function subscribeNotifs() {
  ...
  STATE.notifSubscription = STATE.sb.channel('notifs-' + STATE.user.id)
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: 'user_id=eq.' + STATE.user.id },
      function(payload) { updateNotifBadge(); showToast('🔔 ' + payload.new.text); })
    .subscribe();
}
...
async function updateNotifBadge() {
  ...
  try {
    var r = await STATE.sb.from('notifications').select('id', { count: 'exact' })
...
async function markNotifsRead() {
  if (!STATE.sb || !STATE.user) return;
  try {
    await STATE.sb.from('notifications').update({ read: true })
      .eq('user_id', STATE.user.id).eq('read', false);
  } catch(e) {}
...
async function renderInbox() {
  ...
  if (!STATE.sb || !STATE.user) {
    wrap.innerHTML = "<div style='padding:40px;text-align:center;color:var(--text2)'>Влез за да видиш известията</div>";
    return;
  }

  // Timeout — ако Supabase не отговори за 8 секунди
  var timedOut = false;
  var timeout = setTimeout(function() {
    timedOut = true;
    wrap.innerHTML = "<div style='padding:40px;text-align:center;color:var(--text2)'>⚠️ Грешка при зареждане.<br><span style='font-size:.8rem'>Провери интернет връзката си</span></div>";
  }, 8000);

  try {
    var r = await STATE.sb.from('notifications').select('*')
      .eq('user_id', STATE.user.id)
      .order('created_at', { ascending: false })
      .limit(50);

    clearTimeout(timeout);
    if (timedOut) return; // вече е показано съобщение за грешка

    wrap.innerHTML = '';
    var data = r.data || [];

    if (data.length === 0) {
      wrap.innerHTML = "<div style='padding:40px;text-align:center;color:var(--text2)'>Нямаш известия все още</div>";
      return;
    }
    ...
    data.forEach(function(n) {
      ...
      item.onclick = async function() {
        ...
        var dot2 = item.querySelector('.notif-dot'); if (dot2) dot2.remove();
        if (n.type === 'follow' && n.from_user_id) { openCreatorProfile(n.from_user_id); }
        else if (n.type === 'message' && n.from_user_id) {
          var profile = await getProfile(n.from_user_id);
          openMessages(n.from_user_id, profile ? (profile.name || 'Потребител') : 'Потребител');
        }
    ...
    clearTimeout(timeout);
    if (!timedOut) {
      wrap.innerHTML = "<div style='padding:20px;text-align:center;color:var(--text2)'>⚠️ Грешка при зареждане</div>";
    }
  }
}

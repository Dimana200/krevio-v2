    ...
    list.innerHTML = data.map(n => `
      <div class="note-item" style="padding:16px;border-bottom:1px solid var(--border);display:flex;gap:12px;background:${n.is_read?'transparent':'rgba(201,168,76,.05)'}">
        ...
        <div style="flex:1">
          ...
          <div style="font-size:.7rem;color:var(--text3)">${new Date(n.created_at).toLocaleDateString()}</div>
        </div>
      </div>
    `).join('');
  } else {
    list.innerHTML = '<div style="padding:60px;text-align:center;color:var(--text3)">Нямате нови известия</div>';
  }
}

...
function stopViewTimer(videoId) {
  if (STATE.viewedVideos['_t_' + videoId]) {
    clearTimeout(STATE.viewedVideos['_t_' + videoId]);
    delete STATE.viewedVideos['_t_' + videoId];
  }
}
...
function buildAd(idx, ads) {
  ...
  var c = mk('div');
  c.style.cssText = 'position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:5;text-align:center;padding:24px';
...
// ====== LAZY LOAD НА COUNTS — само при видимо видео ======
async function loadCardCounts(videoId, likeCnt, cmtCnt, likeBtn, saveBtn) {
  if (!STATE.sb) return;
  try {
    // Всички counts в 2 паралелни заявки вместо 3 последователни
    var [likesRes, cmtsRes] = await Promise.all([
      STATE.sb.from('likes').select('id', { count: 'exact' }).eq('video_id', videoId),
      STATE.sb.from('comments').select('id', { count: 'exact' }).eq('video_id', videoId)
    ]);
    if (likeCnt) likeCnt.textContent = fmt(likesRes.count || 0);
    if (cmtCnt)  cmtCnt.textContent  = fmt(cmtsRes.count  || 0);

    // Проверка дали потребителят е харесал/запазил — само ако е влязъл
    if (STATE.user) {
      var [myLike, mySave] = await Promise.all([
        STATE.sb.from('likes').select('id').eq('video_id', videoId).eq('user_id', STATE.user.id).maybeSingle(),
        STATE.sb.from('saves').select('id').eq('video_id', videoId).eq('user_id', STATE.user.id).maybeSingle()
      ]);
      if (myLike.data && likeBtn) likeBtn.classList.add('liked');
      if (mySave.data && saveBtn) saveBtn.classList.add('saved');
    }
  } catch(e) {}
}
...
function buildCard(v, i) {
  ...
  var obs = new IntersectionObserver(function(entries) {
    if (entries[0].isIntersecting) {
      ...
      // Зареди counts само веднъж при видимост — не при buildCard
      if (!countsLoaded) {
        countsLoaded = true;
        loadCardCounts(v.id, likeCnt, cmtCnt, likeBtn, saveBtn);
      }
      ...
      if (nextItem) {
        var nv2 = nextItem.querySelector('.v-player');
        if (nv2 && !nv2.src && nv2._src) { nv2.src = nv2._src; nv2.preload = 'metadata'; }
      }
    } else {
  ...
  if (cAvatar) {
    var ai = document.createElement('img'); ai.src = cAvatar + '?t=1';
    ai.onerror = function() { av.textContent = '🎬'; try { av.removeChild(ai); } catch(e) {} };
    av.appendChild(ai);
  } else { av.textContent = '🎬'; }
  av.addEventListener('click', function(e) { e.stopPropagation(); e.preventDefault(); openCreatorProfile(v.user_id); });

  var cinfo = mk('div', 'v-creator-info');
  var cn = mk('div', 'v-creator-name'); cn.textContent = cName;
  cn.addEventListener('click', function(e) { e.stopPropagation(); openCreatorProfile(v.user_id); });
  ...
  if (v.title || v.description) {
    ...
    if (v.description) {
      var descClean = v.description.replace(/#[\wа-яА-ЯёЁ]+/g, '').trim();
      if (descClean) { var de = mk('div', 'v-desc-text'); de.textContent = descClean; tb.appendChild(de); }
    }
  ...
  if (tags.length > 0) {
    ...
    tags.forEach(function(tag) {
      var t = mk('span', 'v-tag'); t.textContent = tag;
      t.onclick = function(e) { e.stopPropagation(); searchByTag(tag); };
  ...
  // Like — count показваме 0 веднага, ще се обнови при видимост
  var likeBtn = mk('button', 'v-act');
  var likeIco = ico('heart', 20); var likeCnt = mk('span'); likeCnt.textContent = '0';
  ...
  likeBtn.onclick = async function() {
    ...
    if (isLiked) {
      ...
      try { await STATE.sb.from('likes').delete().eq('video_id', v.id).eq('user_id', STATE.user.id); } catch(e) {}
    } else {
      likeBtn.classList.add('liked');
      likeCnt.textContent = parseInt(likeCnt.textContent) + 1;
      try {
        await STATE.sb.from('likes').insert({ video_id: v.id, user_id: STATE.user.id });
        sendNotif(v.user_id, 'like', '@' + (STATE.user.name || STATE.user.email.split('@')[0]) + ' хареса "' + v.title + '"', v.id);
      } catch(e) { likeBtn.classList.remove('liked'); }
    }
  };
  abar.appendChild(likeBtn);

  // Comments
  var cmtBtn = mk('button', 'v-act');
  var cmtIco = ico('message-circle', 20); var cmtCnt = mk('span'); cmtCnt.textContent = '0';
  cmtBtn.appendChild(cmtIco); cmtBtn.appendChild(cmtCnt);
  cmtBtn.onclick = function() { openComments(v.id); };
  abar.appendChild(cmtBtn);
  ...
  saveBtn.onclick = async function() {
    ...
    if (isSaved) {
      saveBtn.classList.remove('saved'); showToast('Премахнато');
      try { await STATE.sb.from('saves').delete().eq('video_id', v.id).eq('user_id', STATE.user.id); } catch(e) {}
    } else {
      saveBtn.classList.add('saved'); showToast('Запазено!');
      try { await STATE.sb.from('saves').insert({ video_id: v.id, user_id: STATE.user.id }); } catch(e) { saveBtn.classList.remove('saved'); }
    }
  };
  abar.appendChild(saveBtn);

...
function showSeekAnim(item,text){
  var d=mk('div');d.textContent=text;
  d.style.cssText='position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);z-index:50;pointer-events:none;font-size:1.3rem;font-weight:900;color:#fff;background:rgba(0,0,0,.55);padding:10px 20px;border-radius:12px;opacity:1;transition:opacity .4s';
  item.appendChild(d);
  setTimeout(function(){d.style.opacity='0';},400);
  setTimeout(function(){try{item.removeChild(d);}catch(e){}},850);
}
